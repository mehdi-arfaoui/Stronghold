...
- Minimal DR governance with `.stronghold/governance.yml`, confirmed service ownership, expiring risk acceptances, custom policy scoping over built-in rules, and governance-aware score comparisons.
- Governance audit events for ownership confirmation, review-due ownership, accepted and expired risk, policy violations, and CLI-driven governance edits.
- Governance workflows across the product with `stronghold governance` commands, governance API routes, dashboard governance summary cards, owner status badges, and risk-acceptance or policy-violation badges on findings.
