...
type ServiceWithContinuity = Service & {
  continuity?: { rtoHours: number; rpoMinutes: number } | null;
};
...
function riskLevelFromFlags(flags: number): DependencyRisk["riskLevel"] {
  if (flags >= 3) return "high";
  if (flags >= 1) return "medium";
  return "low";
}

export function buildDependencyRisks(
  services: ServiceWithContinuity[],
  dependencies: Array<ServiceDependency & { toService?: ServiceWithContinuity | null }>
): DependencyRisk[] {
  ...
  return dependencies.map((dep) => {
    ...
    if (from.continuity?.rtoHours != null && to.continuity?.rtoHours != null) {
      if (from.continuity.rtoHours < to.continuity.rtoHours) {
        risks.push(
          `RTO du service ${from.name} (${from.continuity.rtoHours}h) inférieur au dépendant ${to.name} (${to.continuity.rtoHours}h).`
        );
    ...
    if (from.continuity?.rpoMinutes != null && to.continuity?.rpoMinutes != null) {
      if (from.continuity.rpoMinutes < to.continuity.rpoMinutes) {
        risks.push(
          `RPO du service ${from.name} (${from.continuity.rpoMinutes} min) inférieur à ${to.name} (${to.continuity.rpoMinutes} min).`
        );
