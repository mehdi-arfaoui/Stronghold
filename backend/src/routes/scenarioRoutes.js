...
router.get("/", async (req, res) => {
    try {
        ...
        const scenarios = await prismaClient_1.default.scenario.findMany({
            where: { tenantId },
            include: {
                services: {
                    include: {
...
router.post("/", (0, tenantMiddleware_1.requireRole)("OPERATOR"), async (req, res) => {
    try {
        ...
        const { name, type, description, impactLevel, rtoTargetHours, serviceIds, } = req.body;
        ...
        const scenario = await prismaClient_1.default.scenario.create({
            data: {
                tenantId,
                name: name.trim(),
                type: type.trim(),
        ...
        if (Array.isArray(serviceIds) && serviceIds.length > 0) {
            const ids = serviceIds.map((id) => String(id));
            ...
            if (services.length !== ids.length) {
                return res.status(400).json({
                    error: "One or more serviceIds do not belong to this tenant or do not exist",
                });
            }
            await prismaClient_1.default.scenarioService.createMany({
        ...
        const fullScenario = await prismaClient_1.default.scenario.findUnique({
 ...
 */
router.post("/:id/steps", (0, tenantMiddleware_1.requireRole)("OPERATOR"), async (req, res) => {
    try {
        const tenantId = req.tenantId;
        if (!tenantId) {
            return res.status(500).json({ error: "Tenant not resolved" });
        ...
        const scenario = await prismaClient_1.default.scenario.findFirst({
        ...
        const step = await prismaClient_1.default.runbookStep.create({
            data: {
                tenantId,
                scenarioId: scenario.id,
                order: ord,
