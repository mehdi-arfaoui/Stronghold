...
function addToIndex(index: Map<string, InfraNodeAttrs[]>, key: string | null, node: InfraNodeAttrs): void {
  if (!key) return;
...
function inferFromEventSourceMappings(nodes: InfraNodeAttrs[], lookup: NodeLookup): ScanEdge[] {
  ...
  for (const lambdaNode of lambdaNodes) {
    ...
    for (const mapping of mappings) {
      const eventSourceArn = readString(mapping.eventSourceArn) || readString(mapping.EventSourceArn);
...
function extractEnvReferences(metadata: Record<string, unknown>, lookup: NodeLookup): EnvReference[] {
  const parsed = asRecordArray(metadata.environmentReferences)
    .map((entry) => {
      const varName = readString(entry.varName) || readString(entry.name);
      const value = readString(entry.value) || readString(entry.target) || readString(entry.reference);
  ...
  for (const [rawVarName, rawValue] of Object.entries(envVars)) {
    ...
    } else if ((varUpper.includes('QUEUE') || varUpper.includes('TOPIC')) && resolveNodeByName(maybeName, lookup)) {
...
function resolveEnvReferenceTarget(reference: EnvReference, lookup: NodeLookup): InfraNodeAttrs | null {
...
function inferFromSharedSecurityGroups(nodes: InfraNodeAttrs[]): ScanEdge[] {
  ...
  for (const groupNodes of sgMap.values()) {
    ...
    const computes = groupNodes.filter((node) =>
      [NodeType.VM, NodeType.CONTAINER, NodeType.SERVERLESS, NodeType.APPLICATION, NodeType.MICROSERVICE].includes(
...
function inferFromNetwork(nodes: InfraNodeAttrs[]): ScanEdge[] {
  ...
  for (const groupNodes of subnetMap.values()) {
    ...
    const computes = groupNodes.filter((node) => [NodeType.VM, NodeType.CONTAINER].includes(node.type as NodeType));
  ...
  for (const groupNodes of vpcMap.values()) {
    ...
    const computes = groupNodes.filter((node) =>
      [NodeType.VM, NodeType.CONTAINER, NodeType.APPLICATION, NodeType.MICROSERVICE].includes(node.type as NodeType),
    );
    const dbs = groupNodes.filter((node) => [NodeType.DATABASE, NodeType.CACHE].includes(node.type as NodeType));

    for (const compute of computes) {
      for (const db of dbs) {
        if (compute.id === db.id) continue;
        const alreadyInferred = edges.some((edge) => edge.source === compute.id && edge.target === db.id);
...
function inferFromTags(nodes: InfraNodeAttrs[]): ScanEdge[] {
  ...
  for (const node of nodes) {
    const appTag = node.tags?.app || node.tags?.application || node.tags?.service || node.tags?.project;
    if (!appTag) continue;
...
function deduplicateEdges(existing: ScanEdge[], inferred: ScanEdge[]): ScanEdge[] {
  const existingKeys = new Set(existing.map((edge) => `${edge.source}->${edge.target}:${edge.type}`));
