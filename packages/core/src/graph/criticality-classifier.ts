import type { BlastRadiusResult } from './blast-radius-engine.js';
...
export interface CriticalityClassification {
  readonly tier: number;
  readonly confidence: number;
  readonly signals: string[];
  readonly impactCategory: 'critical' | 'high' | 'medium' | 'low';
}
...
function getCriticalityFromTags(tags: Record<string, string>): number | null {
  ...
  if (critTag) {
    ...
    if (['critical', 'tier1', 'p0', 'p1'].some((k) => lower.includes(k))) return 1;
    if (['high', 'tier2', 'p2'].some((k) => lower.includes(k))) return 2;
    if (['medium', 'tier3', 'p3'].some((k) => lower.includes(k))) return 3;
    if (['low', 'tier4', 'p4'].some((k) => lower.includes(k))) return 4;
  }
...
function getCriticalityFromName(name: string): number | null {
  ...
  if (['prod', 'production', 'primary', 'main', 'master', 'core'].some((k) => lower.includes(k)))
    return 1;
  if (['api', 'web', 'gateway', 'frontend', 'backend', 'app-server'].some((k) => lower.includes(k)))
    return 1;
  if (
    ['staging', 'dev', 'test', 'backup', 'archive', 'log', 'monitoring'].some((k) =>
      lower.includes(k),
    )
  )
    return 4;
  if (['worker', 'batch', 'cron', 'scheduler', 'job'].some((k) => lower.includes(k))) return 3;
  if (['dlq', 'dead-letter', 'error', 'retry'].some((k) => lower.includes(k))) return 4;
  return null;
}
...
export function classifyServiceCriticality(
  ...
  const weightedTier = signals.reduce((sum, s) => sum + s.tier * s.weight, 0) / totalWeight;
