...
function matchesType(nodeText: string, candidates: string[]): boolean {
  return candidates.some((c) => nodeText.includes(normalizeText(c)));
}
...
export function inferBestEffortEdges(
  ...
  const existing = new Set(
    existingEdges.map((edge) => edgeKey(edge.source, edge.target, edge.type)),
  );
  ...
  for (const rule of COMMON_ARCHITECTURE_PATTERNS) {
    const sources = nodes.filter((node) =>
      matchesType(nodeTextById.get(node.id) ?? '', rule.sourceTypes),
    );
    const targets = nodes.filter((node) =>
      matchesType(nodeTextById.get(node.id) ?? '', rule.targetTypes),
    );
    ...
    for (const source of sources) {
      for (const target of targets) {
        ...
        if (rule.edgeType === EdgeType.DEAD_LETTER) {
          ...
          if (
            !targetName.includes('dlq') &&
            !targetName.includes('dead') &&
            !targetName.includes('error')
          ) {
            continue;
          }
        }
