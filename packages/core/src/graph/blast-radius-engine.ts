...
function normalizeEdgeDirection(edge: BlastEdge): NormalizedDependency {
  switch (edge.type) {
    ...
    case EdgeType.DEAD_LETTER:
      return { dependent: edge.targetId, dependency: edge.sourceId };
    ...
    default:
      return { dependent: edge.sourceId, dependency: edge.targetId };
  }
}
...
export function calculateBlastRadius(
  ...
  return serviceNodes.map((node) => {
    ...
    const directDependents = directDependentsSet
      ? Array.from(directDependentsSet).filter((id) => id !== node.id).length
      : 0;
