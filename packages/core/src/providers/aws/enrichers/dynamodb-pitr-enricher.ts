/**
 * Enriches DynamoDB nodes with Point-in-Time Recovery status.
 */
...
function resolveDynamoTableName(
  metadata: Record<string, unknown>,
  fallback: string,
): string | null {
  return (
    readString(metadata.tableName) ?? readString(metadata.dbIdentifier) ?? readString(fallback)
  );
}

export const dynamoDbPitrEnricher: Enricher = {
  ...
  enrich: async (nodes, credentials, region): Promise<EnrichmentResult> => {
    ...
    const getClient = (awsRegion: string): DynamoDBClient => {
      ...
      const resolved = resolveAwsCredentials(creds, awsRegion, 'stronghold-enrichment-dynamodb');
    ...
    for (const node of nodes) {
      ...
      if (!tableName) {
        skipped += 1;
        setNodeMetadata(node, { pointInTimeRecovery: null, pointInTimeRecoveryStatus: null });
        continue;
      }
