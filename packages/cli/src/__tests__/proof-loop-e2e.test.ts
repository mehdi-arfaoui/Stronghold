...
describe('Proof Loop E2E', () => {
  ...
  const cliPath = 'node --import tsx packages/cli/src/index.ts';
  const testTimeoutMs = 60_000;

  function run(command: string): { readonly stdout: string; readonly exitCode: number } {
    try {
  ...
  it('complete proof loop: demo -> UNKNOWN -> evidence -> MET', () => {
    const demo = run('demo');
    ...
    const validate1 = run('contracts validate');
    ...
    const evidence = run(
      `evidence add --service ${serviceName} --scenario region_failure --type tested --rto 45m --rpo 2m`,
    );
    expect(evidence.exitCode).toBe(0);

    const validate2 = run('contracts validate');
    expect(validate2.stdout).toContain('MET');
    expect(validate2.stdout).not.toContain('UNKNOWN');
    expect(validate2.stdout).toContain('45m');
  }, testTimeoutMs);

  it('contracts validate --format json produces valid JSON with MET', () => {
    run('demo');
    const serviceName = readDemoContractService();
    run(
      `evidence add --service ${serviceName} --scenario region_failure --type tested --rto 45m --rpo 2m`,
    ...
    const result = run('contracts validate --format json');
  ...
  it('contracts validate --ci uses [PASS] text', () => {
    run('demo');
    const serviceName = readDemoContractService();
    run(
      `evidence add --service ${serviceName} --scenario region_failure --type tested --rto 45m --rpo 2m`,
    ...
    const result = run('contracts validate --ci');
    expect(result.stdout).toContain('[PASS]');
    expect(result.stdout).not.toContain('\u2713');
  }, testTimeoutMs);

  it('clean state: no pre-existing .stronghold/ contamination', () => {
    const demo = run('demo');
    expect(demo.exitCode).toBe(0);
