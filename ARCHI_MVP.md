...
- **Découverte automatisée** : `POST /discovery/scan` (scan réseau/cloud), `POST /discovery/import` (CSV/JSON), `POST /discovery/github-import` (import depuis dépôt GitHub).
- **Navigation UI** : route frontend `/navigation` pour explorer les modules et filtres.

## Découverte & connecteurs externes (cartographie)
La cartographie est alimentée via :
- imports CSV/JSON (CMDB/NetFlow),
- scans réseau/cloud (à brancher côté worker : SNMP/SSH/WMI, APIs cloud),
- imports GitHub depuis des exports standardisés (JSON) stockés dans un dépôt public.

Connecteurs recommandés à intégrer côté worker (hors MVP) :
- **Réseau** : SNMP (LLDP/CDP), SSH/WMI pour inventaire serveur.
- **Cloud** : AWS/Azure/GCP via API inventaire (instances, VPC/VNet, tags).
- **CMDB** : import API/CSV pour relier services et dépendances.

Le flux attendu est : collecte → export JSON standard → `/discovery/github-import` → suggestions → import final.
